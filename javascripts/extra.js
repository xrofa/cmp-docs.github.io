...
const addElementToCodeBox = (name) => {
  ...
  const closingElement = Array.from(children).find(element => element.outerHTML === '<span class="p">&gt;&lt;/</span>');
...
const removeElementFromCodeBox = (name) => {
  ...
  const elementToRemove = Array.from(children).find(element => element.outerHTML === `<span class="na"> ${SCRIPT_PROPERTIES[name]}</span>`);
